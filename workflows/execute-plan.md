...
@/Users/phillipdougherty/.claude/get-shit-done/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" init execute-phase "${PHASE}")
...
```bash
node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" phases list --type summaries --raw
...
2. Per task:
   ...
   - **MANDATORY refactor-first analysis:** Before writing new code for this task, answer these questions:
     1. Does existing code already handle part of this? If yes, can it be extended/modified instead of writing parallel code?
     2. Would this new code duplicate logic that exists elsewhere? If yes, refactor the existing code to be reusable.
...
| **2: Missing Critical** | Missing essentials per project standards (SECURITY.md, APIS.md, ERROR-HANDLING.md, TESTING-STRATEGY.md): error handling, validation, auth, CSRF/CORS, rate limiting, indexes, logging, idempotent migrations | Add → test → verify → track `[Rule 2 - Missing Critical]` | Auto |
...
| **5: Unnecessary Abstraction** | New file/class/module/wrapper that duplicates or parallels existing code; new abstraction layer without justification | STOP → evaluate refactor alternative → refactor existing code OR justify new code in commit → track `[Rule 5 - Abstraction]` | Auto |
...
**Rule 5 format:**
```
🔍 Refactor Check: [proposed new file/class/module]

Existing code in same domain: [files evaluated]
Can existing code be extended? [yes/no + reasoning]
Decision: [REFACTOR existing | CREATE new — because: justification]
```

**Priority:** Rule 4 (STOP) > Rule 5 (evaluate) > Rules 1-3 (auto) > unsure → Rule 4
**Edge cases:** missing validation → R2 | null crash → R1 | new table → R4 | new column → R1/2 | new utility file → R5 | non-idempotent migration → R2
**Heuristic:** Affects correctness/security/completion? → R1-3. Structural? → R4. New abstraction? → R5. Maybe? → R4.
...
See `/Users/phillipdougherty/.claude/get-shit-done/references/tdd.md` for structure.
</tdd_plan_execution>
...
See /Users/phillipdougherty/.claude/get-shit-done/references/checkpoints.md for details.
</step>
...
node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" state update-progress

# Record execution metrics
node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" state record-metric \
...
node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>

<step name="update_project_standards">
After execution, check if the work introduced patterns that should be reflected in project standards artifacts. Update only when execution introduced NEW patterns not already covered:

- **SECURITY.md** — New auth mechanism, new data protection pattern, new external service integration with security implications
...
If updates made:
```bash
node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs: update project standards after ${PHASE}-${PLAN}" --files [updated artifact files]
```
</step>
...
```bash
node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
