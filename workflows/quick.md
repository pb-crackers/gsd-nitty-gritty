...
```bash
INIT=$(node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" init quick "$DESCRIPTION")
...
- .planning/SECURITY.md (if exists — security standards to follow)
...
- **Refactor-first:** Before creating new files or modules, check if existing code can be extended. Include a `<refactor_check>` field on each task listing existing files in the same domain.
- **Standards compliance:** Reference project standards artifacts (SECURITY.md, APIS.md, etc.) for tasks involving security, APIs, database queries, error handling, or testing.
...
```bash
node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
