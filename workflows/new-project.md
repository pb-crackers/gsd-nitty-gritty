...
**If auto mode:** Skip — go straight to auto-advance.

Everything above captured what the user wants to build. The PM lens is the separate question
of whether it is the right thing to build: who it is for, what bet it is making, which
requirements survive a scope pressure test, what is explicitly not being built, and how
they will know it worked.

It is off by default because it is a real conversation, not a form — perhaps twenty minutes,
and it will push back on features the user likes. Offer it plainly and take no for an answer:

Use AskUserQuestion:
- header: "PM lens"
- question: "Want to pressure-test the scope before building? It challenges each requirement against what you're actually betting on, and usually moves some things to v2."
- options:
  - "Not now" — Start building; you can ask for the PM lens any time
  - "Yes, walk me through it" — About 20 minutes, and expect push-back on scope
  - "I've thought about it" — Capture your answers without the full conversation

**If "Not now":** Note in PROJECT.md under "PM Validation" that the lens was offered and
deferred, with the date. Continue.

**Otherwise:** Read `/Users/phillipdougherty/.claude/get-shit-done/references/pm-lens.md` and
follow it. Do not read that file unless the user accepted — it is ~3.6k tokens and the whole
point of it living outside this workflow is that skipping costs nothing.

A roadmap already exists at this point, so if the lens defers requirements to v2, follow its
`<scope_changed>` instruction and offer to regenerate the roadmap against the refined scope.
...
- [ ] PM lens offered at the end, and either run or recorded as deferred in PROJECT.md
