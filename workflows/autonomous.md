...
```bash
DETAIL=$(node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```bash
PHASE_STATE=$(node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op ${PHASE_NUM})
```
...
# Project standards artifacts (inform grey area proposals with established patterns)
...
- **SECURITY.md** — Established security patterns (use to pre-answer security grey areas)
...
```bash
node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
