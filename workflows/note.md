...
- **Global scope**: `/Users/phillipdougherty/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/phillipdougherty/.claude/notes/`)
...
2. Glob `/Users/phillipdougherty/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/phillipdougherty/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/phillipdougherty/.claude/notes/` — works in any directory
