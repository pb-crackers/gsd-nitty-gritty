...
5. Commit:
```bash
node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_DESC=$(node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" | jq -r '.section')
```
...
**If found:**
1. Read template: `/Users/phillipdougherty/.claude/get-shit-done/templates/VALIDATION.md`
...
```bash
UI_PHASE_CFG=$(node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
## 5.7. Project Standards Artifact Gate

Check that project standards artifacts exist before planning. These documents contain security, API, testing, and error handling standards that the planner MUST reference.
...
Display which are missing and generate them before proceeding. Use PROJECT.md, REQUIREMENTS.md, and any research context to tailor to the project's stack and domain. Follow the artifact templates defined in `new-project.md` Step 7.5.
...
**If all exist:** Continue silently. Set these paths for use in the planner prompt:
```bash
SECURITY_PATH=".planning/SECURITY.md"
...
2. Disable Nyquist with the exact command:
   `node "/Users/phillipdougherty/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow.nyquist_validation false`
...
- {SECURITY_PATH} (Project security standards — MUST reference for auth/validation/data tasks)
...
- Tasks in XML format with read_first, acceptance_criteria, and refactor_check fields (MANDATORY on every task)
- Verification criteria
- must_haves for goal-backward verification
</downstream_consumer>

<refactor_first_principle>
## Refactor-First Principle (MANDATORY)

**The default is REFACTOR. New code requires justification.**

Before creating tasks that introduce new files, modules, classes, or abstraction layers, the planner MUST:

1. **Assess what exists** — Review existing code that touches the same domain. Include these files in `<read_first>`.
2. **Prefer modification over extension** — If a feature can be achieved by modifying existing code, plan a modification task, not a "create new" task.
3. **Justify new code** — If new code IS necessary, document WHY in a `<refactor_check>` field on the task:
   - What existing code was evaluated?
   - Why can't it be extended/refactored to serve this purpose?
   - What would the refactor look like and why is new code better?

4. **Include `<refactor_check>` on EVERY task** — List existing files/modules that touch the same domain. The executor MUST review these before writing new code.

**Examples:**
- BAD: "Create new `src/utils/validation.ts`" (when `src/lib/validators.ts` already exists)
- GOOD: "Extend `src/lib/validators.ts` with email and phone validation functions"
- ACCEPTABLE NEW CODE: "Create `src/services/stripe.ts` — no existing payment integration exists. Evaluated: `src/lib/`, `src/utils/`, `src/services/` — none handle payments."

**Why this matters:** AI-generated code tends toward creating new files and abstractions rather than building on existing code. This inflates codebase size, increases cognitive load, and creates parallel implementations of similar logic. Refactoring first keeps the codebase lean and comprehensible.
</refactor_first_principle>

<first_principles_decomposition>
## First-Principles Decomposition (MANDATORY)

Before creating tasks, decompose the phase goal into its fundamental engineering concerns. For each concern, create targeted tasks. Use this checklist — the planner determines which concerns are relevant per phase and MUST explicitly acknowledge which were evaluated and which were skipped (with reasoning):

- **Data Model** — What data structures/schemas/migrations are needed? What are the relationships and constraints? Are migrations idempotent and safe to re-run? Reference APIS.md Data Access & Migration Safety sections.
- **Security** — Reference SECURITY.md. What auth/authz applies? What input validation? What data protection? What CORS/CSRF policy?
- **API Design** — Reference APIS.md. What endpoints? What request/response shapes? What error cases? What versioning?
- **Error Handling** — Reference ERROR-HANDLING.md. What can fail? How does each failure surface to the user? What error codes?
- **Testing** — Reference TESTING-STRATEGY.md. What's the test plan? Unit vs integration vs E2E boundaries?
...
- Security: Applied — input validation added per SECURITY.md
- API Design: Applied — endpoint follows APIS.md conventions
- Error Handling: Applied — error responses per ERROR-HANDLING.md
- Testing: Applied — unit tests for validation, integration test for endpoint
- Logging: Applied — request logging with correlation IDs
- Performance: N/A — simple single-row lookup
- State Management: N/A — stateless endpoint
```
</first_principles_decomposition>
...
1. **`<read_first>`** — Files the executor MUST read before touching anything. Always include:
   ...
   - Relevant project standards artifacts (SECURITY.md for auth/validation tasks, APIS.md for API/database tasks, etc.)

2. **`<acceptance_criteria>`** — Verifiable conditions that prove the task was done correctly. Rules:
   - Every criterion must be checkable with grep, file read, test command, or CLI output
   - NEVER use subjective language ("looks correct", "properly configured", "consistent with")
   - ALWAYS include exact strings, patterns, values, or command outputs that must be present
   - For security-related tasks: include criteria that verify SECURITY.md compliance (e.g., "passwords hashed with bcrypt", "inputs validated server-side")
   - For API tasks: include criteria that verify APIS.md compliance (e.g., "endpoint uses /v1/ prefix", "error response matches schema")
   - For database tasks: include criteria that verify migration idempotency (e.g., "migration uses IF NOT EXISTS")
   - Examples:
...
4. **`<refactor_check>`** — Existing files/modules in the same domain that the executor MUST review before writing new code. If the task can be accomplished by modifying existing code, the action should say "modify X" not "create new Y". Include justification if new code is necessary.
...
- [ ] Every task has `<read_first>` with at least the file being modified + relevant standards artifacts
- [ ] Every task has `<acceptance_criteria>` with grep-verifiable conditions
- [ ] Every `<action>` contains concrete values (no "align X with Y" without specifying what)
- [ ] Every task has `<refactor_check>` listing existing code in the same domain
- [ ] First-principles decomposition documented (concerns evaluated/skipped with reasoning)
- [ ] Project standards artifacts (SECURITY.md, APIS.md, TESTING-STRATEGY.md, ERROR-HANDLING.md) referenced where applicable
